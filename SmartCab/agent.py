...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        if testing:
            ...
            self.updateEpsilon(funcType= 'alphaPower', should_decay_alpha = False)
            self.trial_num = self.trial_num + 1
        #print "epsilon value is ", self.epsilon
        return None

    def updateEpsilon(self, funcType, should_decay_alpha = False):
        if should_decay_alpha:
            self.alpha = self.alpha - 0.05
        if funcType == 'linear':
            DECREASE_FACTOR = 0.01
            self.epsilon = self.epsilon - DECREASE_FACTOR
        elif funcType == "alphaPower":
            self.epsilon = math.pow(self.alpha, math.sqrt(self.trial_num))
        elif funcType == "cosine":
            print (self.alpha * self.trial_num)
    ...
    def createQ(self, state):
        ...
        if state not in self.Q:
            ...
            for action in self.env.valid_actions:
                self.Q[state][action] = INITIAL_VALUE
        return
    ...
    def choose_action(self, state):
        ...
        # When learning, choose a random action with 'epsilon' probability
            ...
            if self.should_choose_random(epsilon=self.epsilon):
                '''
                EXPLORATION
                We choose a random action with epsilon probability; to do more exploration
                '''
                action = random.choice(self.valid_actions)
            else:
                '''
                ...
                all_actions_with_max_Q = [k for k,v in self.Q[state].iteritems() if v == maxQ]
                action = random.choice(all_actions_with_max_Q)
        return action

    # Source: https://stackoverflow.com/questions/5886987/true-or-false-output-based-on-a-probability
    def should_choose_random(self, epsilon):
